...
int main(int argc, char *argv[]) {
    ...
    if (!arquivo_lido)
    { // verifica se o arquivo foi aberto corretamente, caso contrário, imprime a mensagem de erro.
...
void Descompactador::Cria_Arvore()
{
    ...
    if (this->codigo_Arvore.size() < 1)
    {
        return;
    } // se não houver codigo de arvore, para a funcao
...
uint8_t Descompactador::Obtem_Bit()
{
    if (this->buffer_elementos == 0)
    {
        ...
        if (fread(&byte, 1, 1, this->leitor) == 0)
        {
            return 2;
        } // fread faz com que a variavel byte receba o byte do arquivo, o if compara para ver se o arquivo é valido, caso não for valido, retorna 2 (indicando erro na main)
...
void Descompactador::Obtem_Codigo_Arvore()
{
    ...
    while (contador != this->alfabeto_k)
    {
        ...
        if (bit == 2)
        {
            printf("erro ao capturar bits da arvore");
            return;
        }
    }
}
...
Descompactador::Descompactador(FILE *leitor, FILE *escritor) : leitor(leitor), escritor(escritor), vetor_frequencia(256, nullptr), indice(0, nullptr), alfabeto_k(0), letras_t(0), buffer_elementos(0), codigo_Arvore(0, 0), codigo_No(0, 0)
{
    ...
    {
        this->buffer[i] = 0;
    } // inicializa o buffer com 0 em seus elementos
}
...
void Compactador::descarrega()
{
    ...
    {
        return;
    } // se não houver elementos no buffer, retorna sem escrever
...
int Compactador::obtem_Byte()
{
    ...
    {
        return -1;
    } // caso não houver mais bytes, retorna "-1", quebrando o "while" do distribuidor.
    return byte;
}
